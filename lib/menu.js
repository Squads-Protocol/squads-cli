...
class Menu{
    ...
    transaction = async (tx, ms, txs) => {
        ...
        if (Object.keys(tx.status)[0] !== "draft") {
    ...
    programAuthority = async (ms, currentAuthority, programId) => {
        ...
        if (action) {
            ...
            try {
                ...
                await inquirer.prompt({default: "", name: 'action', type: 'input', message: `Enter to continue`});
                this.multisig(ms);
            }catch(e){
                console.log(e);
                status.stop();
                await inquirer.prompt({default: "", name: 'action', type: 'input', message: `Transaction creation failed - Enter to continue`});
                this.program(ms);
            }
        }else{
            this.program(ms);
    ...
    create = async () => {
        ...
        if (action) {
            ...
            try {
                ...
                status.stop();
                await inquirer.prompt({name: 'action', type: 'input', message: `Created new multisig! (${ms.publicKey.toBase58()}) Press enter to continue...`});
                const squads = await this.api.getSquads(this.wallet.publicKey);
                this.multisigs = squads;
            }catch(e) {
                console.log(e);
                await inquirer.prompt({name: 'action', type: 'input', message: `Error! (${e}) - Enter to continue`});
                status.stop();
            }
            this.top();
        }else{
