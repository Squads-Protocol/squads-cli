...
class API{
    ...
    async createTransaction(msPDA, authorityIndex){
        return this.squads.createTransaction(msPDA, authorityIndex);
    }

    async addInstruction(txPDA, ix) {
    ...
    async activate(txPDA){
        return this.squads.activate(txPDA);
    }

    async getVaultAssets(vaultPDA) {
