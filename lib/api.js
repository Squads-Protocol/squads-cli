...
export const executeTransaction = async (tx) => {
    return squads.executeTransaction(tx);
};

export const approveTransaction = async (tx) => {
    return squads.approveTransaction(tx);
}

export const addKeyTransaction = async (msPDA, key) => {
...
export const removeKeyTransaction = async (msPDA, key) => {
    const txBuilder = await squads.getTransactionBuilder(msPDA, 0);
    const [txInstructions, txPDA] = await (
...
export const changeThresholdTransaction = async (msPDA, threshold) => {
    const txBuilder = await squads.getTransactionBuilder(msPDA, 0);
    const [txInstructions, txPDA] = await (
